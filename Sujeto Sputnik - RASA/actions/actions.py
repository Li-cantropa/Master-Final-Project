...
class LlamaActionAdapter(Action):
    ...
    def _extract_revealed_info(self, intent: str, entities: List, response: str) -> List[str]:
        ...
        #3. Verificar qué información se ha revelado basada en palabras clave
    ...
    def _format_response(self, response, intent):
        ...
        #1. Añadir gestos si no los tiene
        if "*" not in response:
            gestures_by_intent = {
                "greet": [
            ...
            gesture = random.choice(gesture_options) #Elige un gesto aleatorio de las opciones disponibles
            response = f"{gesture}\n\n{response}"
    ...
    def _update_slots(self, tracker, intent, entities, user_message):
        ...
        if intent == "ask_philosophical_question": 
            current_depth = tracker.get_slot("philosophical_depth") or 1 #Se obtiene primero el nivel actual de profundidad
            events.append(SlotSet("philosophical_depth", current_depth + 1)) #Se añade un evento para aumentar el nivel en 1
            ...
            for entity in entities:
                if entity.get('entity') == 'personal_information' and entity.get('value') == 'nombre':
                    ...
                    if name_match:
                        ...
                        events.append(SlotSet("human_name", name_value)) #Se añade un evento para establecer el nombre del humano
            
        return events #Devolvemos la lista de eventos para que se procesen en el tracker
...
class ActionRespondToFallback(LlamaActionAdapter):
    ...
    def create_prompt(self, intent, entities, user_message, tracker):
        ...
        return prompt
        #Aquí no es necesario añadir el estilo, ya que no es un mensaje específico, sino una respuesta a un error o a una pregunta
